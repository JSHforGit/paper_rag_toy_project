...
- ** 완전한 프라이버시**: 모든 처리가 로컬에서 실행 (인터넷 연결 불필요)
...
- ** 사고 과정 표시**: LLM의 추론 과정을 실시간으로 확인 가능
...
| 모델 | 크기 | RAM 요구량 | 용도 | 비고 |
|------|------|-----------|------|------|
| `EXAONE-3.5-2.4B-Instruct` | ~1.5GB | 4GB | 빠른 테스트 | 한국어 특화 |
| `Llama-3.2-3B-Instruct` | ~2GB | 6GB | 일반 사용 | 영어 논문 |
| `Qwen2.5-7B-Instruct` | ~4.5GB | 10GB | 고품질 | 균형잡힌 성능 |
| `Mistral-7B-Instruct` | ~4.5GB | 10GB | 영어 특화 | 빠른 추론 |
...
```
[사이드바 - 모델 파라미터]
- Temperature: 0.1 (정확도 우선) ~ 0.7 (창의성)
- Max Tokens: 
  - 짧게 (512): 간단한 답변
  - 보통 (2K): 일반적인 질문
  - 길게 (Max): 전체 요약/분석
